"""
DAG: dynamic_training_demo
============================
Демонстрирует Dynamic Task Mapping — количество тасков определяется
в runtime, а не при парсинге DAG-файла.

Пайплайн:
    get_models → train(model) ×N → report

Сценарий:
    Первая таска читает список моделей из Airflow Variable (или возвращает
    дефолтный список). Для каждой модели автоматически создаётся отдельный
    таск обучения — параллельно. Финальный таск собирает результаты.

Концепции:
    - Dynamic Task Mapping (.expand) — динамическое количество тасков
    - expand + op_kwargs — маппинг параметров на параллельные таски
    - reduce — финальный таск автоматически ждёт все mapped-таски
    - partial — фиксированные параметры, общие для всех mapped-тасков
"""
